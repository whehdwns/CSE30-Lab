...
void LinkedList_char::insertAtFront(char initialValue)  //works the same way as insertAtBack execpt deals with elements from the front
{ 												 //exercise 2
	Node*temp = new Node();
...
bool LinkedList_char::removeFromFront()   //works the same way as removeFromBack execpt its deals with elements from the front
{ 									//exercise2
	Node*temp = first;
	if (first == NULL){
		return false;
